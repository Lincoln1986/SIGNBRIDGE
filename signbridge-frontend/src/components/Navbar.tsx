  ...
  const navLink = (to: string, label: string) => (
    <button
      ...
      style={{
        ...
        color: pathname === to ? 'var(--violet)' : 'var(--gray-600)',
        padding: '6px 0',
    ...
    <nav style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 36 }}>
        ...
        <div style={{ display: 'flex', gap: 24 }}>
          {isAdmin ? navLink('/admin', 'Panel Admin') : navLink('/dashboard', 'Mi Panel')}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--gray-800)' }}>{user?.full_name}</div>
          <div style={{ fontSize: '0.75rem', color: 'var(--gray-400)' }}>{user?.role}</div>
        </div>
        <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'var(--violet)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 700, fontSize: '0.9rem' }}>
